...
INSERT INTO `dsorder` (`orderID`, `customerID`, `empID`, `carID`, `totalPrice`) VALUES
(1, 2, 1, 'LO44REJ', 2650),
(2, 6, 5, 'FO22FDY', 6280),
(3, 8, 10, 'YL31NLA', 8540),
...
INSERT INTO `dsservice` (`serviceID`, `empID`, `carID`, `dateLastServiced`, `nextService`, `problemdesc`) VALUES
...
--
ALTER TABLE `dscustomer`
  ADD CONSTRAINT `customer vehicle` FOREIGN KEY (`regNo`) REFERENCES `dscar` (`regNo`);
...
ALTER TABLE `dsorder`
  ...
  ADD CONSTRAINT `vehicle purchased` FOREIGN KEY (`carID`) REFERENCES `dscar` (`regNo`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Constraints for table `dsservice`
--
ALTER TABLE `dsservice`
  ADD CONSTRAINT `service vehicle registration` FOREIGN KEY (`carID`) REFERENCES `dscar` (`regNo`) ON DELETE CASCADE ON UPDATE CASCADE,
